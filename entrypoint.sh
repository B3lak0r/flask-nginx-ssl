...
cd /opt/ssl/
openssl genrsa -des3 -passout pass:alucard -out server.pass.key 1024
openssl rsa -passin pass:alucard -in server.pass.key -out server.key
rm server.pass.key
openssl req -new -key server.key -out server.csr -subj "/C=PT/ST=Porto/L=Gaia/O=flask-sample.tf/OU=flask-sample.tf/CN=local.flask-sample.tf"
